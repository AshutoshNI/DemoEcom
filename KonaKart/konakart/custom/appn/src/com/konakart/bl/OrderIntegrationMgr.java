...
public class OrderIntegrationMgr extends BaseMgr implements OrderIntegrationMgrInterface
{
    ...
    private void exportOrder(OrderIf order)
    {
        try
        {
            if (getConfigMgr().getConfigurationValueAsBool(
            ...
            if (order != null && order.getOrderTotals() != null)
            {
                for (int i = 0; i < order.getOrderTotals().length; i++)
                {
                    ...
                    if (ot.getClassName().equals("ot_free_product"))
                    {
                        ...
                        {
                            log.warn("Free Product promotion doesn't contain a product id in the custom2 attribute. The value found was "
                                    + ot.getCustom2() + ". Inventory will not be deducted.");
                        }
                        if (prodId > 0)
                        {
                            getProdMgr().updateProductQuantityWithOptions(prodId, /* opts */null, /* encodedOptions */
