...
/**
 * Called to edit an order by an Admin User. Retrieves the id from the orderId parameter.
 */
public class EditOrderAction extends BaseAction
{
    ...
    public String execute()
    {
        ...
        try
        {
            ...
            /*
             * In the checkout confirmation page the admin user can decide that instead of editing
             * an existing order he wants to insert a new one. We set the archive id of the checkout
             * order to be null and then return to the order confirmation page without resetting the
             * checkout order
             */
            OrderIf checkoutOrder = kkAppEng.getOrderMgr().getCheckoutOrder();
            String newOrder = request.getParameter("removeEdit");
            if (newOrder != null && newOrder.equalsIgnoreCase("true") && checkoutOrder != null)
            {
                checkoutOrder.setArchiveId(null);
